...
object RangeJoinImpl extends Serializable{
  ...
  def overlapJoin(sc: SparkContext,
    ...
    val kvrdd2: RDD[(Long, Iterable[sql.Row])] = rdd2.map(x => (intervalTree.value.getAllOverlappings(x._1), x._2)) //join entry with the intervals returned from the interval tree
